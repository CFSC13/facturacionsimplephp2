...
if($_GET[add]=="ok")
{
	...
	if($_POST[nombre]!="")
	{
		$sql=mysqli_query($con,"insert into tipos_operaciones (nombre) values(lower('$_POST[nombre]'))");
		if(!mysqli_error())
		{
			echo "<script>alert('Registro Insertado Correctamente.');</script>";
			echo "<script>window.location='home.php?pagina=tipos_operaciones';</script>";
		}
			else
			{
...
if($_GET[mod]=="ok")
{
	...
	if($_POST[nombre]!="")
	{
		$sql=mysqli_query($con,"update tipos_operaciones set nombre=lower('$_POST[nombre]') where id=$_POST[id]");
 ...
 <div class="tab-content" id="nav-tabContent">
            ...
            <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
               ...
               <?php
                        ...
                        if($_GET[ver]!=0)
                        {
                            $sql=mysqli_query($con,"select *from tipos_operaciones where id=$_GET[ver]");
                                if(mysqli_num_rows($sql)!=0)
                                {   
                                    $r=mysqli_fetch_array($sql);
                                }
                                $url="home.php?pagina=tipos_operaciones&mod=ok";
                                $showform="show";
                        }
                            else
                            {
                                $url="home.php?pagina=tipos_operaciones&add=ok";
                                $showtable="show";
                            }
                    ?>
                        ...
                        <div id="collapseListado" class="collapse <?php echo $showtable; ?>" aria-labelledby="headingTwo" data-parent="#accordion">
                            <div class="card-body" >
                             <div class="table-responsive" style="padding-right: 1% !important;">
                                    ...
                                    <tbody>
                                        <?php $q=mysqli_query($con,"select * from tipos_operaciones order by nombre"); 
                                            if(mysqli_num_rows($q)!=0){
                                                while($r=mysqli_fetch_array($q)){?>
                                                 <tr>
                                                     <td><?php echo $r['nombre']; ?></td>
                                                     <td><a href="home.php?pagina=tipos_operaciones&ver=<?php echo $r['id'] ?>" title="Editar" alt="Editar"><i class="fas fa-edit icono_editar"></i></a> 
                                                     </td>
                                                 </tr>       
                                             <?php }
                                             }?>       
