...
//echo $targetFile;
	...
	$targetFile = $uploadDir .ids($con,"fotos_productos").".".end($extension);
			...
			//	img(str_replace('//','/',$targetPath),ids("inmu_fotos").".".end($extension),end($extension));								
				img($uploadDir,ids($con,"fotos_productos").".".end($extension),end($extension));								
			//fin de cambio
				
	$q=mysqli_query($con,"insert into fotos_productos values(".ids($con,"fotos_productos").", ".intval($_GET[id]).", '".ids($con,"fotos_productos").".".end($extension)."',0)");
			//nuevo
